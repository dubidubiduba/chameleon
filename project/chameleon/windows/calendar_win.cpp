...
void calendar_win::on_newButton_clicked()
{
...
void calendar_win::on_editButton_clicked()
{
    //获取当前所选日程,日期信息
    //未选中事件则返回
...
void calendar_win::on_deleteButton_clicked()
{
    //确定删除?
    QListWidgetItem* item = ui->listWidget->currentItem();
    int itemrow = ui->listWidget->currentRow();
...
void calendar_win::on_calendar_clicked(const QDate &date)
{
...
void calendar_win::on_colorBox_currentIndexChanged(int index)
{
    if(index == -1)
        return;
...
//锁定窗口
//void calendar_win::on_actionLock_triggered()
//{
//    locked = !locked;
//    //    hide();
...
void calendar_win::mouseMoveEvent(QMouseEvent *event)
{
    ...
    //    }
    calendar_win::mouseMoveEvent(event);
}


void calendar_win::on_checkBox_toggled(bool checked)
{
    dragEnable = checked;
...
void calendar_win::on_languageBox_currentTextChanged(const QString &arg1)
{
    if(arg1 == "English")
    {
        language = arg1;
...
void calendar_win::on_actionSave_triggered()
{
    settings->setValue("language", language);
    settings->setValue("dragEnable", dragEnable);
    settings->sync();
...
bool calendar_win::ReadXML(QString dir)
{
...
bool calendar_win::WriteXML(QString dir)
{
...
void calendar_win::on_actionImport_XML_triggered()
{
    QString dir = QFileDialog::getOpenFileName(this, tr("Select XML"), "/", "(*.xml)");
    ReadXML(dir);
}

void calendar_win::on_actionExport_XML_triggered()
{
